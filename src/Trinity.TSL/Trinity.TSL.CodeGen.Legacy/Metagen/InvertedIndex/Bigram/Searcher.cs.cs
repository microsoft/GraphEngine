...
source.Append(Codegen.GetString(Trinity::Codegen::GetNamespace()));
source.Append(@".InvertedIndex\"", index_name);
            ...
            if (InMemory)
            {
                ...
                using (FileStream fs = new FileStream(index_file, FileMode.Open, FileAccess.Read, FileShare.Read))
                {
                    ...
                    while (index_len > 0)
                    {
                        ...
                        {
                            if (fs.Rea");
source.Append(@"d(reading_buffer, 0, buff_len) == buff_len)
                            {
...
source.Append(@"      }
        ...
        /// </summary>
        /// <param name=""query"">A query string that can contain arbitrary number of wildcard symbo");
source.Append(@"l *.</param>
        ...
        /// <returns>A list of matched cell Ids.</returns>
 ");
source.Append(@"       internal List<long> SubstringSearch(params string[] keywords)
        {
            ...
            for (int i = 0; i < keywords.Length; i++)
            {
                ...
                if (pairList.Count == 0)
                    return new List<long>(0);
                va");
source.Append(@"r _result = SearchSubString4WildcardSearch(pairList);
             ...
             *
             * Successfully");
source.Append(@" going through all the elements in IndexItemList means:
...
source.Append(@" = IteratorList[i];
   ...
   ");
source.Append(@"                         if (distance >= keywords[i - 1].Length)
                            {
                                offset = item.Offset;
                                match = true;
                                break;
                    ...
                    iterator_0++; 
         ");
source.Append(@"   } while (iterator_0 < partialResults[0].Count);
            #endregion
            return Result;
        }
        /// <summary>
...
source.Append(@"                      if (!match)
        ...
        /// <returns></returns>
        private List");
source.Append(@"<IndexItem> SearchSubString4WildcardSearch(List<PairInfo> pairList)
        {
            List<GCHandle> gchandlers = null;
            if (!InMemory)
                gchandlers = new List<GCHandle>();
            ...
            for (int i = 0; i < pairCount; i++)
            {
                if (InMemory)
                    ...
                    IndexItemList[i] = ReadIndexItemListFromDisk(pairList[i].A, pairList[i].B, out ItemCountList[i], ref gchandlers);
");
source.Append(@"                DistanceList[i] = pairList[i].pos - offset;
                offset = pairList[i].pos;
                if (ItemCountList[i] == 0)
                {
                    if (InMemory)
             ...
             *
             * Succ");
source.Append(@"essfully going through all the elements in IndexItemList means:
             * The distance between the current item offset in IndexItemList[i] and
             * the current item offset in IndexItemList[i-1] equals DistanceList[i].
             * ***************************************************************************************************/
            int iterator_0 = 0; 
            ...
            do
            {
                ...
                if (current_item.CellId >= cellId)
                {
                    ...
                    for (; i < pairCount; i++)
                    {
                        ...
                        {
   ");
source.Append(@"                         item = IndexItemList[i][j];
                            if (item.CellId >= current_item.CellId)
                            {
                                break;
                            }
                        ...
                        do
                        {
                            item = IndexItemList[i][j];
                            int dist");
source.Append(@"ance = item.Offset - offset;
                            if (distance == DistanceList[i])
                            {
                                offset = item.Offset;
                                match = true;
...
source.Append(@"                  }
        ...
        private IndexItem* ReadIndexItemListFromDisk(int a, int b, out int count, ref List<GCHandle> gchandlers)
        {
            ...
            return");
source.Append(@" (IndexItem*)gchandle.AddrOfPinnedObject().ToPointer();
        }
