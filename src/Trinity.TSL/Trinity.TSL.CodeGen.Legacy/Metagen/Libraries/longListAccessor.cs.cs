...
");
source.Append(@"        internal static CellIdCache[]    s_IdCache       = new CellIdCache[c_idcache_count];
        ...
        public unsafe long this[int index]
        {
            ...
            {
              ");
source.Append(@"  return *(long*)(CellPtr + (index << 3));
            }
            set
            {
...
source.Append(@">
        ...
        internal unsafe void AddCellID(long cellId, long element)
        {
            ...
            {
                CellPtr = this.ResizeFunction(this.CellPtr - sizeof(in");
source.Append(@"t), sizeof(int), sizeof(long));
...
source.Append(@" << 3;
        ...
        public unsafe void AddRange(longListAccessor collection)
        {
            ...
            if (collection.CellID != CellID)
            {
                CellPtr = ResizeFunction(CellPtr - 4, *(int*)(CellPtr - 4) + 4, delta);
     ");
source.Append(@"           Memory.Copy(collection.CellPtr, CellPtr + *(int*)CellPtr + 4, delta);
                *(int*)CellPtr += delta;
            }
            else
            {
...
source.Append(@"       }
        ...
        /// <returns>true if the List contains on");
source.Append(@"e or more elements that match the conditions defined by the specified predicate; otherwise, false.</returns>
        ...
        public unsafe void CopyTo(long[] array)
        {
         ...
         ");
source.Append(@"   fixed (long* lp = array)
            {
        ...
        public unsafe void CopyTo(long[] array, int arrayIndex)
        {
            ...
            if (array.Length - arrayIndex < Count) throw new ArgumentException(""The number of elements in the source List is greater than the available space from arrayIndex to the end of the dest");
source.Append(@"ination array."");
            fixed (long* lp = array)
            {
...
source.Append(@" 0 || count < 0) throw new ArgumentOutOfRangeException(""arrayIndex is less than 0 or index is less than 0 or count is less than 0."");
...
source.Append(@"cannot be null, but it can contain elements that are null, if type T is a reference type.</param>
        ...
        /// </summary>
        /// <para");
source.Append(@"m name=""index"">The zero-based starting index of the range of elements to remove.</param>
        ...
        /// </summary>
        /// <param name=""a");
source.Append(@"ccessor"">The longList instance.</param>
...
source.Append(@"erAllocator.AllocBuffer((int)targetPtr);
        ...
        /// <");
source.Append(@"returns>true if the value of <paramref name=""a"" /> is the same as the value of <paramref name=""b"" />; otherwise, false.</returns>
...
source.Append(@"Accessor a, longListAccessor b)
        {
            return !(a == b);
        }
        /// <summary>
