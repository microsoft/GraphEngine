...
namespace Trinity.Azure.Storage.Test
{
    ...
    public class DownloadTest
    {
        ...
        {
            var container = m_client.GetBlobContainerClient(BlobStorageConfig.Instance.ContainerName);
            await container.CreateIfNotExistsAsync();
            var dir = container.GetBlobClient(m_version.ToString());
            ...
            await dir.GetBlobClient("0").GetBlobClient(c1.Id.ToString()).UploadFromByteArrayAsync(f1, 0, f1.Length);
            await dir.GetBlobClient("0").GetBlobClient(c2.Id.ToString()).UploadFromByteArrayAsync(f2, 0, f2.Length);
            await dir.GetBlobClient("0").GetBlobClient(Constants.c_partition_index).UploadTextAsync(idx);
            await dir.GetBlobClient(Constants.c_finished).UploadTextAsync("");
