...
namespace Trinity.ServiceFabric.Diagnostics
{
    ...
    public sealed class Log : EventSource
    {
        ...
        public static class Keywords
        {
            ...
            public const EventKeywords Trinity = (EventKeywords)0x4L;
        }
        #endregion
        ...
        private const int TrinityFatalEventId = 7;
        [Event(TrinityFatalEventId, Level = EventLevel.Critical, Message = "{0}")]
        private void TrinityFatal(string message)
        {
            if (this.IsEnabled())
            {
                WriteEvent(TrinityFatalEventId, message);
            }
        }

        private const int TrinityErrorEventId = 8;
        [Event(TrinityErrorEventId, Level = EventLevel.Error, Message = "{0}")]
        private void TrinityError(string message)
        {
            if (this.IsEnabled())
            {
                WriteEvent(TrinityErrorEventId, message);
            }
        }

        private const int TrinityWarnEventId = 9;
        [Event(TrinityWarnEventId, Level = EventLevel.Warning, Message = "{0}")]
        private void TrinityWarn(string message)
        {
            if (this.IsEnabled())
            {
                WriteEvent(TrinityWarnEventId, message);
            }
        }

        private const int TrinityInfoEventId = 10;
